...
stx_build_dblink_spec_type (THREAD_ENTRY * thread_p, char *ptr, DBLINK_SPEC_TYPE * dblink_spec)
{
  ...
  /* join bind push-down */
  ptr = or_unpack_int (ptr, &dblink_spec->join_bind_count);
  if (dblink_spec->join_bind_count > 0)
    {
      ptr = or_unpack_int (ptr, &offset);
      dblink_spec->join_bind_regu_list =
	stx_restore_regu_variable_list (thread_p, &xasl_unpack_info->packed_xasl[offset]);
      if (dblink_spec->join_bind_regu_list == NULL)
	{
	  goto error;
	}
    }
    ...
    {
      dblink_spec->join_bind_regu_list = NULL;
    }

  /* unrelated subquery bind */
  ptr = or_unpack_int (ptr, &dblink_spec->subquery_bind_count);
  if (dblink_spec->subquery_bind_count > 0)
    {
      ptr = or_unpack_int (ptr, &offset);
      dblink_spec->subquery_bind_regu_list =
	stx_restore_regu_variable_list (thread_p, &xasl_unpack_info->packed_xasl[offset]);
      if (dblink_spec->subquery_bind_regu_list == NULL)
	{
	  goto error;
	}
    }
    ...
    {
      dblink_spec->subquery_bind_regu_list = NULL;
    }
